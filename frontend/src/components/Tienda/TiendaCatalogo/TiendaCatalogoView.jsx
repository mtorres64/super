...
import { Search, ShoppingCart, X, Plus, Minus, User, ChevronRight, Star, Package, LogOut, MapPin, ChevronDown } from 'lucide-react';
...
const ProductCard = ({ producto, onAgregar, cantidadEnCarrito, onActualizar, currencySymbol, companyLogo }) => {
  return (
    <div style={{
    ...
    >
      ...
      <div style={{ height: 140, background: 'var(--primary-bg, #ecfdf5)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        {producto.imagen
      ...
      <div style={{ padding: '0.85rem', flex: 1, display: 'flex', flexDirection: 'column', gap: 6 }}>
        ...
        <p style={{ fontSize: '1.05rem', fontWeight: 700, color: PRIMARY, margin: 0 }}>
          ...
          {producto.tipo === 'PESO' && <span style={{ fontWeight: 400, fontSize: '0.75rem', color: '#9ca3af' }}> / kg</span>}
        </p>
        {/* Controles carrito */}
        {cantidadEnCarrito > 0 ? (
...
const DrawerCarrito = ({ carrito, carritoOpen, setCarritoOpen, onActualizar, totalCarrito, onIrAlCheckout, currencySymbol, config }) => {
  return (
    <>
          ...
          {carrito.length === 0 ? (
            ...
            <div key={item.producto_id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: '0.85rem', paddingBottom: '0.85rem', borderBottom: '1px solid #f9fafb' }}>
              <div style={{ flex: 1 }}>
                ...
                <p style={{ fontSize: '0.8rem', color: '#6b7280', margin: 0 }}>{currencySymbol}{item.precio_unitario?.toFixed(2)} c/u</p>
              </div>
...
const SucursalSelector = ({ sucursales, sucursalId, onCambiar }) => {
  ...
  // Auto-seleccionar la primera si no hay ninguna asignada
  useEffect(() => {
      ...
      {open && (
        <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: 'white', border: '1px solid #e5e7eb', borderRadius: 10, boxShadow: '0 4px 16px rgba(0,0,0,0.1)', minWidth: 200, zIndex: 100, overflow: 'hidden' }}>
          ...
          {sucursales.map(s => (
            <button
              key={s.id}
              onClick={() => { onCambiar(s.id); setOpen(false); }}
  ...
  return (
    <div style={{
      ...
      <DrawerCarrito
        ...
        onActualizar={actualizarCantidad} totalCarrito={totalCarrito}
        onIrAlCheckout={onIrAlCheckout} currencySymbol={currencySymbol} config={config}
      />

      {/* FAB carrito mobile (visible cuando hay items y carrito cerrado) */}
